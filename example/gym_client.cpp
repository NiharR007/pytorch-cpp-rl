        ...
        for (int update = 0; update < num_updates; ++update)
        {
            for (int step = 0; step < batch_size; ++step)
            {
                ...
                if (env_info->observation_space_shape.size() > 1) {
                    ...
                    real_rewards = flatten_vector(step_result->reward);
                    
                    // Flatten nested done vector
                    dones_vec.clear();
                    for (const auto& inner_vec : step_result->done) {
                        for (size_t i = 0; i < inner_vec.size(); ++i) {
                            dones_vec.push_back(inner_vec[i]);
                        }
                    }
                } else {
                    ...
                    // Flatten the nested vectors
                    observation_vec = step_result->observation; // Direct access
                    real_rewards = step_result->reward;
                    // dones_vec = step_result->done;
                    
                    // Since done is already a flat vector, directly copy its elements
                    dones_vec.clear();
                    dones_vec.insert(dones_vec.end(), step_result->done.begin(), step_result->done.end());
                }

                // Add debug logging
                spdlog::debug("Received observation size: {}", observation_vec.size());
                spdlog::debug("Received rewards size: {}", real_rewards.size());
                spdlog::debug("Received done size: {}", dones_vec.size());
                ...
                for (int i = 0; i < num_envs; ++i)
                {
                    ...
                    dones[i][0] = static_cast<int>(dones_vec[i]);
                }
